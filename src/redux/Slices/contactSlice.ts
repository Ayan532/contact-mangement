...
const contactSlice = createSlice({
  ...
  reducers: {
    ...
    deleteContact: (state, action: PayloadAction<string | undefined |number>) => {
      state.contacts = state.contacts.filter(contact => contact.id !== action.payload);
      saveContactsToLocalStorage(state.contacts);
    },

    // Action to get a contact by ID
    getContactById: (state, action: PayloadAction<number|string>) => {
      state.contact  = state.contacts.find(contact => contact.id === action.payload) as Contact;
    },
  },
});
